### reboot
Derivative work of autowrap:pyd and pyd (could it possibly be reintegrated?). Provides 
markup (via User Defined Attributes - UDAs) to expose d functions, structs, etc to python 
without requiring redesign of d code, just markup and clarification code.
...
2. structs in pyd don't have full support requiring wrapper classes in D - expose similarly
3. should answer NotImplemented, and elegantly throw TypeError where appropiate 
    e.g. `str+BType` and `4**BType` are okay but `1+BType` and `BType**4` should cause type errors as does 1+str
4. have a clear public api
5. can we loose the *
