...
import pyd.reboot._dispatch_utils : supportsNArgs, minArgs, maxArgs, gensym;
...
//
//    static if(RebootFullTrace && LOG) pragma(msg, "terneryfunc!"~fname~" = "~signatureWithAttributes!fn);
//
...
ReturnType!fn callFuncArgsKwargsReturnDType(alias fn, string fname)(PyObject* args, PyObject* kwargs) {
    ...
    static if (MAX_FN_ARGS.vstyle == Variadic.no && MIN_FN_ARGS == 0){
        static if(RebootFullTrace && LOG) pragma(msg, "--------- MAX_FN_ARGS.vstyle == Variadic.no && MIN_FN_ARGS == 0");
    ...
    foreach(i, arg; t.fields) {
        ...
        static if(RebootFullTrace && LOG) pragma(msg, "--------- argNum"~argNum.stringof);

        static if(MAX_FN_ARGS.vstyle == Variadic.no){
            static if(RebootFullTrace && LOG) pragma(msg, "--------- MAX_FN_ARGS.vstyle == Variadic.no");
            ...
            static if (argNum >= MIN_FN_ARGS && (!MAX_FN_ARGS.hasMax || argNum <= MAX_FN_ARGS.max)){
                static if(RebootFullTrace && LOG) pragma(msg, "--------- (argNum >= MIN_FN_ARGS && (!MAX_FN_ARGS.hasMax || argNum <= MAX_FN_ARGS.max))");
            ...
            static if(RebootFullTrace && LOG) pragma(msg, "--------- (MAX_FN_ARGS.vstyle == Variadic.typesafe)");
